...
class SpotEnv:
    def __init__(self, num_obs = 52, num_actions = 12, num_commands = 4, show_viewer=True, device="cuda", num_steps_per_ep = 2000, mode = 'sample', model = None, data = None):
    ...
    def rewards(self, current_action):
        # === GREATLY INCREASE FOOT LIFTING REWARDS ===
        ...
        action_penalty       =  -0.002 * self._reward_action_rate(current_action)  # Reduced
        orientation_penalty  =  -0.01  * self._reward_roll_pitch_()  # Reduced
        sliding_penalty      =  -0.1   * self.feet_slide_np()  # GREATLY reduced from -0.3
        ang_vel_xy_penalty   =  -0.01  * self._reward_ang_vel_xy()  # Reduced
